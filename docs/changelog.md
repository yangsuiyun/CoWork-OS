...
### Added
- **Codex Security scan workflows**: bundled the Codex Security plugin pack with repository-wide, scoped-path, diff, and deep multi-pass security scan workflows; added directory-backed plugin-pack skill loading for `SKILL.md` folders with references/scripts/assets; added workspace-local scan artifact orchestration, deep-scan worker artifact checks, deterministic round inventory merge, and report validation/render helpers. See [Codex Security Scans](codex-security-scans.md).
...
### Changed
- **Plugin-pack packaging and skill metadata**: bundled plugin-pack resources now package all files under `resources/plugin-packs/**`, and directory-backed plugin-pack skills use manifest metadata first, then `SKILL.md` frontmatter, then generated skill titles.
...
### Fixed
- **Codex Security scan helper containment**: `security_scan_*` helpers are hidden from normal tasks, rejected outside Codex Security tasks, constrained to the active workspace, protected against traversal in `scan_id` and scoped paths, and deep-scan merges now require exactly six usable workers with parse-valid JSONL artifacts.
